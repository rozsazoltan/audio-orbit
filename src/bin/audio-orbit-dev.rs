...
fn main() {
    ...
    println!("Audio Orbit dev runner");
    ...
    loop {
        ...
        if let Some(app) = child.as_mut() {
            match app.try_wait() {
                Ok(Some(status)) => {
                    println!("Audio Orbit exited with {status}.");
                    child = None;
                }
                Ok(None) => {}
                Err(error) => {
                    eprintln!("failed to poll Audio Orbit process: {error}");
                    child = None;
                }
            }
        }
...
fn build_app() -> bool {
    println!("building audio-orbit...");

    let status = Command::new("cargo")
        .args(["build", "--bin", "audio-orbit"])
        .stdin(Stdio::null())
        .status();
    ...
    match status {
        ...
        Ok(status) => {
            eprintln!("build failed with {status}; waiting for the next file change.");
            false
        }
        Err(error) => {
...
fn start_app(root: &Path) -> Option<Child> {
        ...
        .spawn()
    {
        ...
        Err(error) => {
            eprintln!("failed to start Audio Orbit: {error}");
            None
        }
    }
}
...
fn stop_child(child: &mut Option<Child>) {
    ...
    match app.try_wait() {
        ...
        Ok(None) => {}
        Err(error) => {
            eprintln!("failed to poll previous Audio Orbit process: {error}");
            return;
        }
    }

    println!("stopping previous Audio Orbit instance...");
    if let Err(error) = app.kill() {
        eprintln!("failed to stop previous Audio Orbit instance: {error}");
    }
    let _ = app.wait();
}
