...
/// contribution. The companion vector is kept for cached-state compatibility and
/// possible analysis features, but the UI renders a clean gray/blue/yellow bar.
...
impl LiveSpectrumAnalyzer {
    ...
    pub fn push_sample(&mut self, sample: f32) -> Option<SpectrumBucket> {
        ...
        let level = smooth_attack_release(adaptive_level, self.previous_bucket.level, 0.34, 0.10);

        let bucket = SpectrumBucket {
            level: level.clamp(0.0, 1.0),
            low: smooth_attack_release(target.low, self.previous_bucket.low, 0.28, 0.12),
            mid: smooth_attack_release(target.mid, self.previous_bucket.mid, 0.28, 0.12),
            high: smooth_attack_release(target.high, self.previous_bucket.high, 0.30, 0.12),
        };
    ...
    fn normalize_live_level(&mut self, level: f32) -> f32 {
        ...
        let floor = self.adaptive_floor.min(0.18);
        let peak = self.adaptive_peak.max(floor + 0.22);
        let normalized = ((level - floor) / (peak - floor)).clamp(0.0, 1.0);
        normalized.powf(1.28) * 0.86
    }
}
...
impl SpectrumAnalyzer {
    ...
    fn analyze_window(&mut self, samples: &[f32]) -> SpectrumBucket {
        ...
        // AIMP-like overview bars should mostly communicate perceived loudness, not
        // paint every kick/transient as a full-height spectrum column. Keep FFT data for
        // analysis, but shape the visible bar height around RMS/loudness with a little peak.
        let level = loudness * 0.56
            + min_max_span * 0.22
            + peak.powf(0.92) * 0.16
            + strongest_band * 0.04
            + contrast * 0.015
        ...
        SpectrumBucket {
            level: level.clamp(0.0, 1.0).powf(1.14),
            low: profile.low,
            mid: profile.mid,
            high: profile.high,
        }
